...
import {
    REKOMENDASI_PRODUCT_FAIL,
    REKOMENDASI_PRODUCT_REQUEST,
    REKOMENDASI_PRODUCT_SUCCESS,
    ALL_PRODUCT_REQUEST,
    ALL_PRODUCT_SUCEESS,
    ALL_PRODUCT_FAIL,
...
const getRecommendedProducts = (userId) => async (dispatch) => {
  try {
    dispatch({ type: REKOMENDASI_PRODUCT_REQUEST });
    // console.log(userId)
    const response = await axios.get(`/api/v1/interactions/recommendation/${userId}`);
    // console.log("data Rekomendasi: ", response.data)

    dispatch({
      type: REKOMENDASI_PRODUCT_SUCCESS,
      payload: {
        products: response.data.recommendations,
        productCount: response.data.recommendations.length || 0,
      },
    });
  } catch (error) {
    dispatch({
      type: REKOMENDASI_PRODUCT_FAIL,
      payload: error.response?.data?.message || "Terjadi kesalahan",
    });
  }
};

const getAllProducts = (keyword = "", currentPage = 1, price = [200, 10000], category, ratings = 0, userId = null) => async (dispatch) => {
    try {
        dispatch({ type: ALL_PRODUCT_REQUEST });
...
export { getRecommendedProducts, getAllProducts, clearError, getSingleProducts, newReview, productForAdminPanel, addNewProduct, deleteProduct, updateProduct, getAllProductReviews,deleteProductReviews }